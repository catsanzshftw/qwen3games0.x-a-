...
# Tuned for feel, closer to the original
GRAVITY = 0.75
JUMP_POWER = -15.0
WALK_SPEED = 5.0
RUN_SPEED = 7.5
...
# --- Music Configuration ---
SAMPLE_RATE = 22050 # Lower sample rate for simpler generation
CHANNELS = 1 # Mono sound
BUFFER_SIZE = 512 # Sound buffer size
# --- End Music ---
...
# --- Simple Music Player ---
# This section generates and plays music notes in real-time.
# Based on Pygame sndarray examples and chiptune principles.

def generate_square_wave(frequency, duration, sample_rate=SAMPLE_RATE, volume=0.1):
    """Generates a square wave numpy array."""
        ...
        if period > 0:
            ...
            for i in range(frames):
                if (i % period) < half_period:
                    arr[i] = int(volume * 32767)
                else:
                    arr[i] = int(-volume * 32767)
    return arr

def generate_sine_wave(frequency, duration, sample_rate=SAMPLE_RATE, volume=0.1):
...
def play_note(frequency, duration, volume=0.1, wave_type="square"):
    """Plays a single note."""
    if wave_type == "sine":
        note_array = generate_sine_wave(frequency, duration, volume=volume)
    else: # Default to square
        note_array = generate_square_wave(frequency, duration, volume=volume)
    sound = pygame.sndarray.make_sound(note_array)
    sound.play(-1) # Loop indefinitely
    return sound

# --- Placeholder for a simple melody (e.g., Overworld theme intro) ---
# Frequencies for notes (Hz) - Simplified for demonstration
...
def music_player():
    """A generator that yields (frequency, duration) tuples for a simple melody."""
    # Simple Overworld-like theme intro
    melody = [
        ('E5', 0.2), ('E5', 0.2), ('REST', 0.2), ('E5', 0.2),
        ('REST', 0.2), ('C5', 0.2), ('E5', 0.2), ('G5', 0.2), # Measures 1-2
        ('REST', 0.4), ('G4', 0.4), ('REST', 0.4), # Measure 3
        ('C5', 0.2), ('REST', 0.2), ('G4', 0.2), ('REST', 0.2), ('E4', 0.2), ('REST', 0.2), # Measure 4
        # Add more notes or loop a section for continuous music
        # For now, it will loop this short intro
    ]
...
class Coin:
    ...
    def update(self, frame):
        if self.collected and self.collected_timer < 30:
             self.collected_timer += 1
        elif self.collected_timer >= 30:
             pass
        else:
...
class MarioForever:
    ...
    def handle_collisions(self):
        ...
        for block in self.level.blocks:
            ...
            block_rect = pygame.Rect(block.x, block.y, TILE_SIZE, block_rect_height)
        ...
        for enemy in self.level.enemies:
            ...
            enemy_rect = pygame.Rect(enemy.x, enemy.y, enemy.width, enemy.height)
            for block in self.level.blocks:
                block_rect = pygame.Rect(block.x, block.y, TILE_SIZE, TILE_SIZE)
    ...
    def draw_world_map(self):
        ...
        for i in range(4):
            ...
            if i < 3:
                pygame.draw.line(self.screen, WHITE, (x + 20, preview_y),
                               (x + 80, preview_y), 3)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if current_note_sound is None or current_time - last_note_time > note_duration * 1000:
                try:
                    ...
                    if current_note_sound:
                        current_note_sound.stop() # Stop previous note
                    # Use sine wave for a slightly smoother melody
                    current_note_sound = play_note(freq, dur, volume=0.1, wave_type="sine")
                except StopIteration:
                    # If melody ends, restart (or load a new one)
                    music_generator = music_player()
                    # Continue to next iteration to get the first note of the restarted melody
                    continue
            # --- End Music Update ---
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        self.state = GameState.MENU
                        # Stop music when exiting to menu (optional)
                        # if current_note_sound:
            ...
            if self.state == GameState.MENU:
                ...
                continue_text = self.small_font.render("Press SPACE to continue",
                                                     True, WHITE)
